...
public:
    ...
    void displaydetails()
    {
        ...
        while (getline(fin, name))
        {
            ...
            if (cnic == this->getcnic())  // 🟢 Match by CNIC instead of CandidateID
            {
    ...
    void candidateMenu(candidate &c)
    {
        ...
        do
        {
            ...
            cout << "2. View Vote Count in Election\n";
            cout << "3. See Area Standing\n";
            cout << "0. Log Out\n";
...
public:
    ...
    void unblockUser()
    {
        ...
        ofstream tempFile("temp_block.txt");
        ofstream outFile(targetFile, ios::app);

        if (!blockFile || !tempFile || !outFile)
        {
            cout << Red << "File open error.\n" << Reset;
            return;
        }
                ...
                if (index >= 5 && record[4] == targetRole)
                {
                    ...
                    if (choice == 'y' || choice == 'Y')
                    {
                        for (int i = 0; i < index; i++)
                            outFile << record[i] << endl;
                        outFile << "----------" << endl;
                        cout << Green << targetRole << " unblocked successfully.\n" << Reset;
                        found = true;
                    }
                    else
                    {
                        for (int i = 0; i < index; i++)
                            tempFile << record[i] << endl;
                        tempFile << "----------" << endl;
                        cout << Yellow << targetRole << " remains blocked.\n" << Reset;
                    }
                }
                else
                {
        ...
        remove("block.txt");
        rename("temp_block.txt", "block.txt");

        if (!found)
        {
            cout << Yellow << "No blocked " << targetRole << "s found.\n" << Reset;
        }
    }
};
        ...
        while (getline(file, name))
        {
            ...
            else if (role == "Candidate") 
            {
                ...
                if (entered_id == candidateID) 
                {
                    ...
                    if (!correctPassword) 
                    {
                        cout << Red << "Too many wrong attempts. Returning to main menu.\n\n" << Reset;
                        break;
                    }
